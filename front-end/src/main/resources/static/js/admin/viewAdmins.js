//    ==================================================== GET ACTIVITY ================================================
const myUrl = `http://localhost:5002/select/auth`;
...
function fillTbody(item, index) {
    ...
    a1.setAttribute("onclick", `deleteAdmin(${item.id})`);
    a1.textContent = "slet";
    td.appendChild(a1);
}
...
function deleteAdmin(id) {
    if (confirm("Vil du slette adminen ?")) {
        // alert("test")
        const requestOptions = {
            'content-type': 'application/json',
            method: 'DELETE',
        ...
        const url = `http://localhost:5002/delete/admins/${id}`
        console.log(id)
